...
export class AuthUserDetailsComponent extends PageComponent implements OnInit, OnDestroy {
  ...
  avatarPending = false;
  uploadAvatarFailed = false;
  pendingFieldNama: string | null = null;
  ...
  constructor(private readonly _changeDetectorRef: ChangeDetectorRef,
    ...
    super();
    const patchUserStart$ = this._actions$.pipe(
      ofType(patchUserStart),
      map((action) => action.data.fieldName),
      takeUntil(this._destroy$)
    );

    patchUserStart$.subscribe((fieldName) => {
      this.pendingFieldNama = fieldName;
      _changeDetectorRef.markForCheck();
    });

    merge(
      patchUserStart$.pipe(sample(this._actions$.pipe(ofType(patchUserSuccess)))),
      this._noPatching$
    ).pipe(takeUntil(this._destroy$)).subscribe(() => {
      this.pendingFieldNama = null;
      _changeDetectorRef.markForCheck();
    });
  ...
  ngOnInit(): void {
    ...
    this._actions$.pipe(
      ofType(postAvatarFailed),
      takeUntil(this._destroy$)
    ).subscribe(() => {
      this.uploadAvatarFailed = true;
      this._changeDetectorRef.markForCheck();
    });
