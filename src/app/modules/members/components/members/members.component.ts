  ...
  animations: [
    ...
    trigger('home', [
      transition(':enter, void => complete', [
        query('.members__item', fadeInAnimation('600ms', '200ms'), { optional: true })
      ])
    ]),
    trigger('home2', [
      transition('state1 => void', [
        group([
          query('.members__item:nth-of-type(odd)', fadeOutLeftAnimation('300ms', '100ms'), { optional: true }),
          query('.members__item:nth-of-type(even)', fadeOutRightAnimation('300ms', '100ms'), { optional: true }),
        ])
      ]),
      transition('state2 => void', [
        group([
          query('.members__item:nth-of-type(3n)', fadeOutRightAnimation('300ms', '100ms'), { optional: true }),
          query('.members__item:nth-of-type(3n+1)', fadeOutLeftAnimation('300ms', '100ms'), { optional: true }),
          query('.members__item:nth-of-type(3n+2)', zoomOutAnimation('300ms', '100ms'), { optional: true }),
        ])
      ]),
      transition('state3 => void', [
        group([
          query('.members__item:nth-of-type(4n+1)', fadeOutLeftAnimation('300ms', '100ms'), { optional: true }),
          query('.members__item:nth-of-type(4n+2)', fadeOutLeftAnimation('300ms', '250ms'), { optional: true }),
          query('.members__item:nth-of-type(4n+3)', fadeOutRightAnimation('300ms', '250ms'), { optional: true }),
          query('.members__item:nth-of-type(4n)', fadeOutRightAnimation('300ms', '100ms'), { optional: true }),
        ])
      ]),
    ])
  ]
...
export class MembersComponent extends PageComponent implements OnDestroy {
  ...
  constructor(private readonly _activatedRoute: ActivatedRoute,
    ...
    this.currentInputSelection = this._activatedRoute.snapshot.queryParamMap.get(QPM_GENDER) ?? 'all';
    ...
    const likeStart$ = this._actions$.pipe(
      ofType(likeMemberFromMembersPageStart),
      map((action) => action.index),
      takeUntil(this._destroy$)
    );

    likeStart$.subscribe((index) => {
      console.log(index);
  ...
  onGenderSelected(value: string) {
    ...
    this.currentInputSelection = value;
    const page = 1;
    const gender = value === 'all' ? null : value;

    this._router.navigate([], { relativeTo: this._activatedRoute, queryParams: { page, gender } });
  }

  onPageSelected(page: number): void {
    if (this.currentInputSelection === 'all') {
      ...
      this._router.navigate([], {
        ...
        queryParams: {
          page,
          gender: this.currentInputSelection
        }
      });
    }
  }
