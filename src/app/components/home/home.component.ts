...
const sliderAnimationMetadata = trigger('slider', [
  ...
  transition('void => big_view_port', [
    group([
      query('.keen-slider__slide:nth-of-type(odd)', [
        fadeInDownAnimation.startState,
        fadeInDownAnimation('500ms', '200ms')
      ], { optional: true }),
      query('.keen-slider__slide:nth-of-type(even)', [
        fadeInUpAnimation.startState,
        fadeInUpAnimation('500ms', '200ms')
      ], { optional: true })
    ])
  ]),
  transition('big_view_port => void', [
    group([
      query('.keen-slider__slide:nth-of-type(odd)', [
        fadeOutDownAnimation.startState,
        fadeOutDownAnimation('500ms', '0ms')
      ], { optional: true }),
      query('.keen-slider__slide:nth-of-type(even)', [
        fadeOutUpAnimation.startState,
        fadeOutUpAnimation('500ms', '0ms')
      ], { optional: true })
    ])
  ])
]);
...
const swipeInfoAnimationMetadata = trigger('swipeInfo', [
  transition(':enter', [fadeInAnimation.startState, fadeInAnimation('300ms', '300ms')]),
