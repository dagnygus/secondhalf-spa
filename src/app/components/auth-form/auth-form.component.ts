...
import { signinStart, signinSuccess, signinFailed } from 'src/app/state/auth/auth.actions';
...
import { FormComponent } from '../../my-package';
...
const btnGroupEnterAnimation = query('.register-form__btn-group', [
  bounceInAnimation.startState,
  bounceInAnimation('600ms', '400ms')
], { optional: true });

const btnGroupLeaveAnimation = query('.register-form__btn-group', [
  bounceOutAnimation('600ms', '0ms')
], { optional: true });

const formHeaderEnterAnimation = query('.register-form__header', [
  fadeInAnimation.startState,
  fadeInAnimation('400ms', '300ms')
], { optional: true });

const formHeaderLeaveAnimation = query('.register-form__header', [
...
export class AuthFormComponent extends FormPageComponent<RegisterModelImpl> implements OnInit, OnDestroy {
  ...
  readonly signUpHttpErrorResponse$: Observable<FirebaseError>;
  ...
  constructor(private readonly _chandeDetectorRef: ChangeDetectorRef,
    ...
    super();
    this.signUpHttpErrorResponse$ = this._actions$.pipe(ofType(signupFailed), map(({ error }) => error));
  }
