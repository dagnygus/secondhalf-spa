...
@Component({
  selector: 'app-user-detail[patchingUserSoruce][userPatchedSource]',
...
export class UserDetailComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @Input() label = '';
  @Input() value = '';
  @Input() patchingUserSoruce!: Observable<string>;
  @Input() userPatchedSource!: Observable<string>;
  ...
  ngOnInit(): void {
    ...
    this.patchingUserSoruce.pipe(
      filter(fieldName => fieldName === this.name),
