...
import { flipInXAnimation, fadeInUpAnimation, flipOutYAnimation, fadeOutLeftAnimation, flipOutXAnimation, fadeOutDownAnimation, fadeOutUpAnimation } from './../../utils/ng-animations';
// eslint-disable-next-line max-len
import { fadeInAnimation, fadeInLeftAnimation, fadeInRightAnimation, fadeOutAnimation, flipInYAnimation } from './../../utils/ng-animations';
...
const fadeAnimationMetadata = trigger('fade', [
  transition(':enter', [ fadeInAnimation.startState, fadeInAnimation('400ms', '300ms') ]),
  transition(':leave', [  fadeOutAnimation('400ms', '300ms') ])
]);

const photoContainerAnimationMetadata = trigger('photoContainer', [
  transition(':enter', [
    flipInYAnimation.startState,
    flipInYAnimation('1s', '400ms')
  ]),
  transition(':leave', [
    flipOutYAnimation('1s', '0ms')
  ])
]);

const headerAnimationMetadata = trigger('header',[
  transition(':enter', [
    query('.single-member-header__name-container', [
  ...
  transition(':leave', [
    group([
      ...
      query('.single-member-header__image-container', [
        fadeOutAnimation('300ms', '100ms')
      ])
    ])
  ]),
]);
